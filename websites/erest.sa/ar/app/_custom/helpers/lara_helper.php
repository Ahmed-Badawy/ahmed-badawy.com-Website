...
//    if( $user_data && !empty($user_data) ) die("true");
//    else die('false');
    if( $user_data && !empty($user_data) ) return true;
    return false;
}
...
function is_admin(){
    $user_data = Session::get('user_data');
    if( $user_data && !empty($user_data) && $user_data->is_admin ) return true;
    return false;
}

function user_data($what){
    dd("ok");
    $user_data = Session::get('user_data');
//    dd($user_data);
    return $user_data->$what;
}
