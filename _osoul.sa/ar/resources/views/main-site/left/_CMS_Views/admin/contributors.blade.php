...
@foreach($contributors as $con)
	<div class="contr">
		...
		<div class="col-sm-10">
			<form method='post' action="{{url('admin/modify-id/contributor/'.$con->id)}}" enctype="multipart/form-data">
				...
				<a id='contributers_createModalButton' class='btn btn-default' href="#contributers_create" data-toggle='modal' role='button'>{!! fa2('plus-square') !!}</a>
...
<a myhref="{{url('admin/delete-id/contributor/'.$con->id)}}" class='btn btn-sm btn-danger delete_button'>{!! fa2('remove') !!} أحذف</a>


			</form>
		</div>
		<img src="{{get_it("site-imgs/_CMS/contributors/".$con->img)}}" class='col-sm-2' />
...
@include(get_location('_CMS_Views.admin.modals.contributor_create_modal'))
