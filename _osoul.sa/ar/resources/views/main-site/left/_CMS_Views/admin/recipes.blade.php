...
<div class="panel-heading">
    <h1 class="panel-title"></h1><h2 class='text-center'>مطبخ أصول</h2>
</div>
...
@foreach($recipes as $con)
		  <hr>

		<div class="col-sm-10">
			<form method='post' action="{{url('admin/modify-id/recipe/'.$con->id)}}" enctype="multipart/form-data">
				<input type='text' class='form-control widthfix' name="item_name" value="{{$con->name}}">
				<textarea rows='4' class='form-control widthfix' id="editor1" name='item_desc'>{{$con->desc}}</textarea>



				<a id='recipes_createModalButton' class='btn btn-default' href="#recipes_create" data-toggle='modal' role='button'>{!! fa2('plus-square') !!}</a>
...
<a myhref="{{url('admin/delete-id/recipe/'.$con->id)}}" class='btn btn-sm btn-danger delete_button'>{!! fa2('remove') !!} أحذف</a>


			</form>
		</div>
		<img src="{{get_it("site-imgs/_CMS/recipes/".$con->img)}}" class='col-sm-2' />
...
@include(get_location('_CMS_Views.admin.modals.recipes_create_modal'))
