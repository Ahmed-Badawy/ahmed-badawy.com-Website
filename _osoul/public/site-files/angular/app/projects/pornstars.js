...
var site_base = "../ng-temp/Hidden/pornstars/";
var partial_base = "../ng-temp/Hidden/pornstars/partials/";
...
myApp.controller('pornstars_controller',function($scope,$http,$routeParams,$timeout){
	...
	var get_data = function(get_page,q){
		$scope.loader = true;
		if(q==null || q=='') url = "pornstars/get/"+get_page;
		else url = "pornstars/get/"+get_page+"/"+q;
	...
	$scope.li_class = function(index){
		if(index=="last") if($scope.pagi && $scope.current_page>=$scope.pagi.pages_number) return "disabled btn btn-danger";
		if(index=="first") if($scope.current_page<=1) return "disabled btn btn-danger";
		return;
	}
