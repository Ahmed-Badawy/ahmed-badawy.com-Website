var js_data =[

{
    q1:"Ahmed-Badawy.com",         
    q2:`This is my personal website for testing & documenting purposes.`,         
    link:['Ahmed-Badawy.com','ahmed-badawy.com/site'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 1
},{
    q1:"Spidro.com",         
    q2:`I co-founded Spidro.com company. 
It's main focus is in hosting web Apps & developing desktop applications for small & medium stores.
We provided applications for (restaurants - pharmacies - selling & buying systems - entrepreneur systems)
I was the leading man in the development operations also did most of the UI & UX of our applications.
`,         
    link:['Spidro.com','spidro.com'],
    cate:"Websites",            
    tags: ["Websites","Worked-at"],
    priority: 2
},{
    q1:"My Technical Blog",         
    q2:`For some reason i don't remember i kept a wordpress blog in mind. 
it's actually well structured & designed`,         
    link:['My Blog','ahmed-badawy.com/blog'],
    cate:"Websites",            
    tags: ["Websites","Blog"],
    priority: 3
},{
    q1:"Enjaz",         
    q2:`Web Development company at Saudi Arabia & gulf area.
I worked there for 8 month. I designed & develped their company website & created many websites & web applications for them.`,         
    link:['Enjaz Saudi Company','erest.sa'],
    cate:"Websites",            
    tags: ["Websites","Worked-at"],
    priority: 4
},{
    q1:"DRK.com.sa Publication Assistance Company",         
    q2:`Publication company at Saudi Arabia.
I developed their website & designed their whole Systems. 
I built agreat Billing Managment System for them. Also created lots of auto bots that helped them reach their clients.`,         
    link:['Dar research and knowledge','drk.com.sa'],
    cate:"Websites",            
    tags: ["Websites","Worked-at"],
    priority: 5
},{
    q1:"Osoul Chicken Manufacturers",         
    q2:`Chicken Manufacturer at Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Osoul.sa Chicken Manufacturers','osoul.sa'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 6
},{
    q1:"Alaryafest for Constructions",         
    q2:`Constructions Managment company at Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Alaryafest.sa for Constructions','alaryafest.sa'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 7
},{
    q1:"Basaffar Medical Establishment (BME)",         
    q2:`Medical Group Managment company at Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Basaffar Medical Establishment (BME)','basaffar-gp.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 8
},{
    q1:"Mostasharak (LTD)",         
    q2:`Constructions Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Mostasharak (LTD)','mostasharak.co'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 9
},{
    q1:"Derby Coffee International",         
    q2:`A Coffe that have 85 branches all over the Middle East. 
    list of their branches: <a href='http://derbycof.com/ar/page/branches' target='_blank'>http://derbycof.com/ar/page/branches</a>
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Derby Coffee International','derbycof.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 10
},{
    q1:"New-Line Constructions",         
    q2:`Constructions Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['New-Line Constructions','newline-sa.net'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 11
},{
    q1:"Ramaint Constructions",       
    q2:`Constructions Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['ramaint Constructions','ramaint.net'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 12
},{
    q1:"Alhada Perfume",       
    q2:`Perfume Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Alhada Perfume','alhadaperfume.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 13
},{
    q1:"Store Managment System",       
    q2:`Store Managment
A develpment module for fast deployment store with brands & products.`,         
    link:['Store Managment System','time-choice.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 14
},{
    q1:"Distinction Diamond for Contracting (DDC)",       
    q2:`Distinction Diamond for Contracting (DDC)
Construction- Maintenance – Air-conditioning Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['DDC Website','didco.co'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 15
},{
    q1:"My Pleasure",       
    q2:`Cultural Events Organizer in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['My Pleasure','mypleasure1.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 16
},{
    q1:"a-low.net",       
    q2:`Legal Consultants Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Ahmed Al-Owaid','a-low.net'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 17
},{
    q1:"alshargawi.com",       
    q2:`lawyers & Legal Consultants Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    // link:['Ahmed Al-Owaid','alshargawi.com'],
    link:['Ahmed Al-Owaid','alsharqawilawfirm.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 17
},{
    q1:"Enjaz Public Services",       
    q2:`Public Services Facilitators in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Enjaz Public Services','enjazoffice.com.sa'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 18
},{
    q1:"Alhabrdy For Shipment",       
    q2:`Shipment Services provider in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Alhabrdy Shipment Services','alhabrdy.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 19
},{
    q1:"EngineeringMagic",       
    q2:`EngineeringMagic Creative Marketing Solutions in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Engineering Magic','engineering-magic.com'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 20
},{
    q1:"Hayat El-Omran",       
    q2:`Constructions Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Hayat El-Omran','b5t.info'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 21
},{
    q1:"Alsahary Realstate",  
    q2:`Constructions Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Alsahary Realstate','alsahary.com/realstate'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 23
},{
    q1:"Alsahary Security",  
    q2:`Security Services Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Alsahary Security','alsahary.com/security'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 24
},{
    q1:"Alsahary Cars",  
    q2:`Rental Car Services Company in Saudi Arabia.
I developed their website & Backend admin control panels to control the website's.`,         
    link:['Rental Cars','alsahary.com/cars/'],
    cate:"Websites",            
    tags: ["Websites"],
    priority: 25
},










{
    "logo":'text-height',
    q1:"Improved Text Editor",
    "page_name_ar":"التحكم النصى",
    q2:"It's a new way to edit site's content without the familiar textarea boxes, It Uses contentEditable HTML 5 API & uses json cashes in the backend.",
    "page_desc_ar":"هى طريقة تسمح لك بالتلاعب بالصفحة دون الحاجة لأى GUI من أى نوع. وهى أسرع طريقة لتعديل نصوص المواقع أو الصفحات الخاصة بك.",
    "page_img":"projects/text.png",
    "link":['Demo','ahmed-badawy.com/site/projects/dynamic'],
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 26      
},

{
    "logo":'eye',
    "page_title":"Video Book Project",
    q1:"Visual Book Project",
    "page_name_ar":"مشروع الكتاب المرئى",
    q2:`Bring books to life with a video & voice reader, also can be translated in multiple languages.
    can be used to learn new languages or create children's interactive movie`,
    "page_desc_ar":"هى فكرة لربط فيديو بنص للأغراض المختلفة مثلا(قصص أطفال - فيديوهات تعليمية - أفلام أنترنت مترجمة بعدة لغات)",
    "page_img":"projects/video-book.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/video-book'],
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 27      
},

{
    "logo":'file-text',
    "page_title":"Quran !",
    q1:"Holy Quran !",
    "page_name_ar":"برنامج القرءان الكريم",
    q2:"it's an idea to attach books to a voice, also can be used to view the books in a different languages with some comments & props added to it.",
    "page_desc_ar":"هى فكرة لربط صوت بنص لقرائة الكتب المختلفة. و عمل playlist لمجموعة من الصوتيات يتم تحميلها بشكل متوازى عبر الـ Ajax. الإستخدمات: عمل Playlist من مجموعة أغانى أو صوتيات تعليمية - أو قرائة كتاب ما بشكل صوتى",
     "page_img":"projects/quran.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/quran'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 28        
},


{
        "logo":'play-circle-o',
        "page_title":"Movies Indexer",
        q1:"Box Office Lister",
        "page_name_ar":"قاموس أفلام الـ Box Office",
        q2:"An app that lists all BoxOffice movies from 1980 to know. it's based on a small replicator i developed earlier that automatically replicates any website's content.",
        "page_desc_ar":"برنامج يحتوى على جميع بيانات أفلام البوكس أوفيس من عام 1980 إلى الآن. و يمكنك التصفية و الإختيار كما تريد.",
        "page_img":"projects/movies.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/movies'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 29          
},

{
      "logo":'star-o',
      "page_title":"Actors Rate",
      q1:"Actors Rater",
      "page_name_ar":"ترتيب الممثلين",
      q2:"simple Rating system that enables you to rate character. Also can be used in any other things like movies or what ever.",
      "page_desc_ar":"هو برنامج يمكنك من عرض بيانات و التصويت للممثلين أو الشخصيات المرغوب فيها. ويمكنك البحث عن شخصيات معينة.",
      "page_img":"projects/actorsrate.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/actors'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 30         
}, 

{
    "logo":'sign-in',
    "page_title":"Remote Sign in",
    q1:"Sign-In manager",
    "page_name_ar":"إثبات الشخصية (فيس-جوجل-تويتر)",
    q2:"code to login using (facebook,google,twitter,github) written fully in PHP.",
    "page_desc_ar":"تطبيق لنظرية الـ Open-Id فى الدخول للمواقع عن طريق حسابات موحدة. تم التنفيذ بإستخدام (Google-Facebook-Twitter-Github)",
    "page_img":"projects/key.jpg",
    link:['Demo','ahmed-badawy.com/site/spidro/login'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 31        
},

{
      "logo":'book',
      "page_title":"Book Reader",
      q1:"Online PDF Reader",
      "page_name_ar":"قارئ كتب الـ PDF",
      q2:`reads pdf books online instead of downloading a pdf reader on your device.
      can be buggy in some cases`,
      "page_desc_ar":"هو عبارة عن تطبيق لـ كود الـ pdf.js لقراءة الكتب عبر الويب.",
      "page_img":"projects/pdf.png",
    link:['Demo','ahmed-badawy.com/site/public/pdf/web/viewer.php?book_name=va7jUIrOhpuA.pdf'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 32         
},

{
      "logo":'credit-card',
    "page_title":"Social ID",
    q1:"Egyption Social-ID Analyzer",
    "page_name_ar":"تحليل بيانات البطاقة الشخصية",
      "q2":`this will analyze the social id card number & extract data from it.`,
      "page_desc_ar":"يحلل رقم البطاقة الشخصية لأى شخص للحصول على بعض البيانات عن هذا الشخص.",
      "page_img":"projects/social-id.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/social-id'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 33         
},

{
      "logo":'heart',
      q1:"Favorite Websites",
      "page_name_ar":"برنامج المواقع المفضلة",
      "page_title":"Favorite Sites",
      "q2":"Saves your favorite websites & there sub pages- & order them with number of access to them",
      "page_desc_ar":"يحفظ البرنامج المواقع المفضلة لك و يسجل عدد مرات دخولك إليها. ويرتب المفضلات لك حسب بياناتك و بيانات المستخدمين الآخرين للبرنامج.",
      "page_img":"projects/heart.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/favorite'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 34            
},
{
      "logo":'check-square-o',
      q1:"ToDo List",
      "page_name_ar":"برنامج قائمة المهام",
      "page_title":"Todo List Manager",
      q2:"A very basic todo manager",
      "page_desc_ar":"هذا البرنامج يسجل المهام المراد القيام بها. و يتيح لك التحكم بها بعد ذلك. يعمل كـ To-Do Manager.",
      "page_img":"projects/todo.png",
      "link":'ahmed-badawy.com/site/projects/todo',
    link:['Demo','ahmed-badawy.com/site/projects/todo'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 35        
},

{
      "logo":'users',
      "page_title":"Box Office Actors",
      q1:"Actor's Movies",
      "page_name_ar":"قاموس ممثلين أفلام الـ Box Office",
      q2:"Lists actor's movies over the years",
      "page_desc_ar":"ملف به كثير من الممثلين و أفلاهم على مدى السنوات. ويمكنك أيضا البحث عنهم فى جوجل أو صورهم. وكذلك الربط بمواقع التواصل الإلكترونى المختلفة.",
      "page_img":"projects/actors.jpg",
      "link":'ahmed-badawy.com/site/projects/movies#/actors',
    link:['Demo','ahmed-badawy.com/site/projects/movies#/actors'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 36       
}, 
{
      "logo":'clock-o',
      q1:"Count Down Manager",
      "page_name_ar":"العد التننازلى",
      "page_title":"CountDown Generator",
      q2:'Save a countdown then allows you to share it with your friends',
      "page_desc_ar":'برنامج يسمح لك يتسجيل تاريخ ثم يصنع عد تنازلى حتى هذا التاريخ. ويسمح لك بمشاركة أصدقائك هذا البرنامج.',
      "program_full_desc":"This is a program that Lets you choose a date for an event in your life then creates a countDown to this date. also it enable you to share this Event-CountDown with your friends on social networks like facebook. هذا برنامج يسمح لك بإختيار تاريخ و موعد لحادث ما فى حياتك ثم يسمح لك بعمل صفحة عد تنازلى لهاذا الموعد. و يمكنك مشاركة صفحة العد التنازلى لأصدقائك على مختلف الشبكات الإجتماعية مثل الفيسبوك.",
      "page_img":"projects/countdown.png",
      "link":'ahmed-badawy.com/site/projects/countdown/index',
    link:['Demo','ahmed-badawy.com/site/projects/countdown/index'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 37        
},
{
            "logo":'cubes',
            q1:"Units Converter",
            "page_name_ar":"حاسبة تحويلات الوحدات",
            "page_title":"Conversion Calculators",
            "page_desc":"Converts between different units",
            "page_desc_ar":"تستخدم للتحويل بين الوحدات المختلفة.",
            "page_img":"projects/converter.png",
    link:['Demo','ahmed-badawy.com/site/projects/converter'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 38                
},
{
      "logo":'dollar',
      q1:"Currency Calculator",
      "page_name_ar":"برنامج تحويل العملات",
      "page_title":"Currency Calculator",
      "q2":"Converts between different currencies",
      "page_desc_ar":"برنامج يحسب التحويلات بين المعملات المختلفة.",
      "page_img":"projects/dollar.png",
    link:['Demo','ahmed-badawy.com/site/projects/currency'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 39         
},
{
      "logo":'money',
      q1:"Advanced Currency Conveter",
      "page_name_ar":"برنامج تحويلات العملات المطور",
      "page_title":"Advanced Currency Calculator",
      q2:"Advanced Currency Converter",
      "page_desc_ar":"يحسب التحويلات بين العملات المختلفة. تطوير لبرنامج سابق.",
      "page_img":"projects/currency-advanced.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/currency-advanced'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 40        
},
{
      "logo":'search',
      "page_title":"Postal Codes Query",
      q1:"Egypt's Postal Codes",
      "page_name_ar":"البحث عن كود البريد",
      q2:"View all postal code data related to a city or a street (egypt only)",
    "page_desc_ar":"البحث عن أكواد البريد و بيانات مكاتب البريد فى مصر, وكذلك الأرقام البريدية لهم. يوضح لك بيانات مفصلة بجميع البيانات المتعلقة بمكاتب البريد المراد الإستفسار عنها.",
      "page_img":"projects/postal-offices.jpg",
    link:['Demo','ahmed-badawy.com/site/projects/postal#/postalcodes'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 41         
},


{
      "logo":'search-plus',
      "page_title":"Postal Offices Query",
      q1:"Search For Postal Offices",
      "page_name_ar":"البحث عن بيانات مكاتب البريد",
      q2:"Search all postal office's data",
      "page_desc_ar":"يبحث عن جميع بيانات و خواص مكاتب البريد بداخل مصر.",
      "page_img":"projects/postal-code.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/postal#/postaloffices'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 42        
}, 


{
      "logo":'clock-o',
      q1:"Percentage Calculator",
      "page_name_ar":"حاسبة النسب",
      "page_title":"Percentage Calculator",
      q2:"Save a persentage then enabels you to use it where ever you want",
      "page_desc_ar":"عليك إدخال النسب و على البرنامج تقسيم المبالغ الواردة بعد ذلك. الإستخدامات(حالات الميراث - حلات المشاركات المالية) ",
      "page_img":"projects/persentage.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/persentage'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 43        
 },
{
      "logo":'flash',
      q1:"Electric Consumption Calculator",
      "page_name_ar":"حاسبة الإستهلاك الكهربائى",
      "page_title":"Electricity Consumption Calculator",
      q2:"An app to calculator electrical consumption bill inside egypt",
      "page_desc_ar":"برنامج لحساب السحب الكهربائى بالكيلو وات و تحديد كم الكهرباء المسحوبة و تكلفتها تبعا لآخر قرارات الوزارة.",
      "page_img":"projects/elec.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/electricity'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 44        
},

{
      "logo":'coffee',
      q1:"Water Consumption Calculator",
      "page_name_ar":"حاسبة الإستهلاك المياه",
      "page_title":"Water Consumption Caclulator",
      q2:"An app to Calculator water consumption bill inside egypt",
      "page_desc_ar":"برنامج لحساب لإستهلاك المياه.",
      "page_img":"projects/water.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/water'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 45        
},
{
      "logo":'money',
      q1:"Money Calculation Helper",
      "page_name_ar":"Money Calculator Helper",
      "page_title":"Money Calculator",
      q2:`Helps You to count money using the number of papers in each currency value.`,
      "page_desc_ar":"برنامج يساعد على عد النقود. Helps You to count money using the number of papers in each currency value. ",
      "page_img":"projects/money-count.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/money-calc'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 46         
},
{
      "logo":'calculator',
      q1:"Numbers System Converter",
      "page_name_ar":"التحويلات بين الأنظمة الرقمية",
      "page_title":"Number Converters",
      q2:"Converts Between HexaDecimal,Binary,Octal,Decimal Numbering Systems",
      "page_desc_ar":"التحويل بين الأنظمة الرقمية المختلفة (العشرى-الثنائى-الثمانى-السداسى عشر).",
      "page_img":"projects/nums.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/converter#/number'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 47         
}, 
{
      "logo":'book',
      q1:"Book Sharing System",
      "page_name_ar":"نظام مشاركة الوثائق",
      "page_title":"Book Sharing System",
      q2:"Enabels you to read & share books. You can choose to download the book or read it online inside the browser.",
      "page_desc_ar":"موقع لمشاركة الكتب مع أصدقائك - و قراءتها على الويب دون الحاجة تحميلها - و أيضا يساعدك على حفظ الكتب و الوثائق و تنظيمها.",
      "page_img":"projects/books.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/books'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 48            
},

{
      "logo":'car',
      q1:"Oil Prices Calculator",
      "page_name_en":"برنامج حساب أسعار الوقود",
      "page_title":"Oil Calculator",
      q2:"As a solution to fast oil prices changes in egypt, this application shows you what's the price of oil today",
      "page_desc_en":"برنامج لحساب أسعار البنزين بعد الزيادة. أدخل الألتار يحسب الثمن - أو العكس. (بنزين 80 - 92 - 95 - سولار - غاز)",
      "page_img":"projects/oil.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/oil'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 49         
},{
      "logo":'bank',
      q1:"Bank Profit Calculator",
      "page_name_ar":"برنامج حساب أرباح البنوك",
      "page_title":"Banks Profit Calculator",
      q2:"Just add the monthly benefit you agreed on with the bank",
      "page_desc_ar":"يحسب أرباح البنك - يشترط أن تدخل الفائدة الشهرية المتفق عليها مع البنك.",
      "page_img":"projects/bank.jpg",
    link:['The APP','ahmed-badawy.com/site/projects/bank'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 50         
},{

      "code":"new-invoice",
      "logo":'cc-visa',
      q1:"Spidro Selling Manager",
      "page_name":"برنامج إدارة المخازن و المحلات",
      q2:`
       this is a selling manager that can be used inside small & medium stores to sell & bill the clients. 
      `,
      "ar_desc":"هو عبارة عن برنامج يمكنك من البيع و التحكم فى محلك عن طريق قاعدة بيانات إلكترونية. يتم الآن العمل على ربطه بآلة قراءة الليبل و طابعة الفواتير و الإيصالات.",
      "page_img":"selling-manager/icon-imgs/selling-manager.png",
    link:['Selling Manager','ahmed-badawy.com/site/selling-manager/new-invoice'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 51         
},{
  "logo":'user',
  q1 : "Advanced Login & Registration System",
  "page_title" : "Registration Area",
  q2 : "This is a Highly effective Registration System ready to be implemented on any site.",
  "page_img" : "projects/register.jpg",
  "link" : 'regist',
    link:['The APP','ahmed-badawy.com/site/registration/index'],    
    cate:"Web APP",            
    tags: ["Applications","web Apps"],  
    priority: 52     
}









{
      "code":"thumb-gene",
      "logo":'camera',
      "page_title":"Thumb Generator",
      "page_name":"مولد الصور المصغرة",
      "page_desc":"تحجيم الصور المختلفة. يصلح للتصغير أو التكبير. صمم خصيصا ليعمل كـ Thumb Generator API.",
      "page_img":"api/thumb.png",
      "link":url('api/thumb-view'),
},    
{
      "code":"captcha-gene",
      "logo":'minus-square',
      "page_title":"Captcha Generator",
      "page_name":"مولد صور الـ Captcha",
      "page_desc":"توليد جمل الكابتشا بالخطوط و الطرق المختلفة. يمتاز بسهولة إستخدامه و سرعته و مرونته فى توليد الأكود. تم تصميم API كامل لتوليد الأكواد لأى موقع على الويب.",
      "page_img":"api/captcha.png",
      "link":url('api/captcha-view'),
},   
{
      "code":"qr-gene",
      "logo":'qrcode',
      "page_title":"QR Generator",
      "page_name":"مولد كود الـ QR",
      "page_desc":"يولد كود QR. يمتاز بسهولة إستخدامة و التحكم فيه. تم تصميم API كامل للتحكم بكيفية رسم الكود.",
      "page_img":"api/qr.png",
      "link":url('api/qr-gene-view'),
},
{
      "code":"upc-gene",
      "logo":'barcode',
      "page_title":"BarCode Generator",
      "page_name":"مولد كود الـ UPC باركود",
      "page_desc":"تم تصميم API كامل ليعمل مولد لـأكواد الـ UPC/BarCode المختلفة بأنواعها.",
      "page_img":"api/upc.png",
      "link":url('api/upc-view'),
}, 















{
            "code":"Rip Page",
            "logo":'file-code-o',
            "page_title":"Rip Page",
            "page_name":"برنامج نزع المحتوى  - Rip Page ",
            "page_desc":`
يمكنك هذا البرنامج من نسخ روابط و صور أى صفحة مفتوحة فى المتصفح و يمكنك أيضا من تحميل روابطهم<br>
<span class='english'>Select images & links on a page and download them.</span>
`,
            "page_img":"external/chrome/rip-page.png",
            "link" : "https://chrome.google.com/webstore/detail/rip-page/gddceihbefbhkgjfbfodkmfchbfnbpho"
},{
            "code":"Search It",
            "logo":'search',
            "page_title":"Search It",
            "page_name":"برنامج البحث على الأنترنت - Search It",
            "page_desc":`
يمكنك هذا البرنامج من البحث عن أى عنصر على الأنترنت بدون أى عناء -
يمكنك إضافة جميع المواقع التى تريد إستخدامها و من ثم البحث بها بدون أى عناء.
الآن يكنك البحث فى أكثر من الف موقع بضغطة واحدة.
أبحث عن كلمة-صورة-رابط-برنامج-أو أى شىء فى الوجود بضغطة واحدة.
<br>
<span class='english'>Search Any Thing & Every Thing in seconds.
Now you can seach any website you want in matter of seconds.</span>
            `,
            "page_img":"external/chrome/search-it.png",
            "link" : "https://chrome.google.com/webstore/detail/search-it/ohgeojakicmbfeimcnfmmdolgaiokfoa"
},{
            "code":"Currency Converter",
            "logo":'dollar',
            "page_title":"Currency Converter",
            "page_name":"برنامج تحويل العملات النقدية - Currency Converter",
            "page_desc":`
الآن يمكنك معرفة سعر العملات من المنزل بضغطة واحدة <br>
 بأقل من ثانيتين يمكنك التحويل لكل العملات المعروفة
    <br>
    <span class='english'>Converts Between all Currencies with one second</span>
            `,
            "page_img":"external/chrome/currency-converter.png",
            "link" : "https://chrome.google.com/webstore/detail/currency-converter/denbpnajoaoppcacfhopcgfjokliemgk"
},{
            "code":"porn preventer",
            "logo":'venus-mars',
            "page_title":"Porn Preventer",
            "page_name":"برنامج منع المواقع الإباحية - Porn Preventer",
            "page_desc":`
يمكنك من منع فتح المواقع الإباحية على متصفح الكروم. - يمنع فتح الموقع و يحولك لموقع إسلامى
<br>
<span class='english'>Preventes your brwoser from opening any porn website.</span>
            `,
            "page_img":"external/chrome/porn-preventer.png",
            "link" : "https://chrome.google.com/webstore/detail/porn-preventer/kjakednfpkbgpmifghjchpiekadckjad"
},{
            "code":"Share It",
            "logo":'share-alt',
            "page_title":"Share It",
            "page_name":"Share It - برنامج المشاركة لأى محتوى على مواقع السوشيال كلها",
            "page_desc":`
البرنامج عبارة عن لستة تم إضافتها على قائمة المهام (الزر الأيمن) أثناء إستخدام المتصفح تسمى Share it. وتمكنك من مشاركة أى محتوى من أى صفحة على الأنترنت لكل مواقع التواصل الإجتماعية المعروفة.
كيف يمكننى أستخدامة:
أدخل البرنامج من الرابط المحدد بالأعلى ثم أدخل لأى موقع يمكنك مشاركة أى شىئ تريده (رابط الصفحة الحالية-صورة-نص محدد-فيديو-صوت) بالضغط زر قائمة المهام بالموس(الفأرة) على العنصر المارد مشاركته ستجد زر share it.
ماذا يمكنك أن تشارك:-
رابط الموقع الحالى - أى صورة بالموقع - أى نص محدد بالموقع - أى فيديو أو صوت بالموقع.

المواقع المدمجة بالبرنامج:
Facebook , Twitter , Google+ , Linked-In , Pinterest , Tumblr , StumbleUpOn , VK , Xing , Reddit , Or enven Send An Email
<br>
<span class='english'>Know You can share every thing & any thing to all known social media websites.</span>
            `,
            "page_img":"external/chrome/share-it.png",
            "link" : "https://chrome.google.com/webstore/detail/share-it/ighbcekeolbcaimlofkjjmnkejlnelaf"
},{
            "code":"Units Converter",
            "logo":'cubes',
            "page_title":"Units Converter",
            "page_name":"Units Converter - برنامج تحويل الوحدات",
            "page_desc":`
برنامج مهمته التحويل بين الوحدات المختلفة
- وحدات الطول - وحدات الضغط - وحدات الوزن - وحدات الكثافة - و وحدات المساحة - وحدات الحجم - وحدات الحرارة - وحدات الطاقة - وحدات السرعة
<br>
<span class='english'>convertes between different units.</span>
            `,
            "page_img":"external/chrome/units-converter.png",
            "link" : "https://chrome.google.com/webstore/detail/units-converter/nmjgjihebpbkoclkpnbfjblgbhdnbgag"
},{
            "code":"Egyptian Social ID Analyzer",
            "logo":'credit-card',
            "page_title":"Egyptian Social ID Analyzer",
            "page_name":"Egyptian Social ID Analyzer - محلل البطاقة المصرية",
            "page_desc":`
يحلل لك بيانات البطاقة المصرية
            `,
            "page_img":"external/chrome/social-id.png",
            "link" : "https://chrome.google.com/webstore/detail/تحليل-البطاقة-المصرية/cbfdcgeeokgfhcbknlbnbllfjechjffe"
},{
            "code":"SnowMan Game",
            "logo":'credit-card',
            "page_title":"SnowMan",
            "page_name":"SnowMan Game",
            "page_desc":`
Build Your Own SnowMan
            `,
            "page_img":"external/chrome/snowman.png",
            "link" : "https://chrome.google.com/webstore/detail/snowman-game/heibcnhfbgplijjflkgfcdhfidneikeb"
},









{
            "code":"text_img",
            "logo":'image',
            "page_title":"Text on Image Generator",
            "page_name":"Text Image Constructor",
            "page_name_ar":"برنامج الكلام على الصور",
            "page_desc":"generates a text of your choice over a predefined image",
            "page_desc_ar":"يولد نص فوق صورة",
            "page_img":"games/text-img.jpg",            
            "link": url("games/text-image"),
},{
            "code":"memory",
            "logo":'square-o',
            "page_title":"Memory Game",
            "page_name":"Memory Game",
            "page_name_ar":"لعبة الذاكرة",
            "page_desc":"test your memory",
            "page_desc_ar":"أمتحن ذاكرتك مع لعبة الذاكرة",
            "page_img":"games/memory.jpg",            
            "link": base_url("htmls/games/memory-game.html"),
},{
            "code":"chess",
            "logo":'street-view',
            "page_title":"Chess Game",
            "page_name":"Play Chess",
            "page_name_ar":"لعبة الشطرنج",
            "page_desc":"simple chess game",
            "page_desc_ar":"ألعب شطرنج ليل نهار اونلين.",
            "page_img":"games/chess.jpg",            
            "link": url("games/chess"),
},{
            "code":"maze",
            "logo":'medium',
            "page_title":"Maze",
            "page_name":"لعبة المتاهة البسيطة",
            "page_desc":"وصل النحلة لبيتها.",
            "page_img":"games/bee.jpg",            
            "link":url('games/maze1'),
},{
            "code":"DrawingApp",
            "logo":'pencil-square',
            "page_title":"Drawing App",
            "page_name":"Drawing App",
            "page_name_ar":"برنامج الرسام",
            "page_desc":"draw with the drawing app",
            "page_desc_ar":"برنامج الرسام لرسم الأشكال المختلفة",
            "page_img":"games/drawing.png",            
            "link": base_url("htmls/games/drawing-app/drawing-app.html"),
},{
            "code":"DrawingApp2",
            "logo":'pencil',
            "page_title":"Drawing APP 2",
            "page_name":"Drawing APP 2",
            "page_name_ar":"برنامج الرسام 2",
            "page_desc":"برنامج الرسام لرسم الأشكال المختلفة",
            "page_desc_ar":"برنامج الرسام لرسم الأشكال المختلفة",
            "page_img":"games/drawing.png",            
            "link": base_url("htmls/games/drawing-app-2.html"),
},{
            "code":"coloringApp",
            "logo":'paint-brush',
            "page_title":"Coloring App",
            "page_name":"Coloring App",
            "page_name_ar":"برنامج التلوين",
            "page_desc":"برنامج يسمح للأطفال بتلوين صورة ما.",
            "page_img":"games/duck.png",            
            "link": base_url("htmls/games/html5-canvas-drawing-app/html5-canvas-drawing-app.html"),
},{
            "code":"coloringApp2",
            "logo":'paint-brush',
            "page_title":"Coloring APP 2",
            "page_name":"Coloring APP 2",
            "page_name_ar":"برنامج التلوين 2",
            "page_desc":"برنامج يسمح للأطفال بتلوين صورة ما.",
            "page_img":"games/coloring.png",            
            "link": base_url("htmls/games/coloring-app-2/coloring-app.html"),
},{
            "code":"dotts_connecting",
            "logo":'dot-circle-o',
            "page_title":"Dottes Connection Game",
            "page_name":"Dottes Connection Game",
            "page_name_ar":"لعبة توصيل النقاط",
            "page_desc":"لعبة توصيل النقاط",
            "page_img":"games/dottes.jpg",            
            "link": base_url('htmls/games/dott-connecting-app/dott-connecting.html'),
},{
            "code":"iceman",
            "logo":'location-arrow',
            "page_title":"Create Ice Man",
            "page_name":"Build Your ICEMan",
            "page_name_ar":"لعبة صنع رجل الجليد",
            "page_desc":"لعبتنا لصنع رجل الجليد",
            "page_img":"games/iceman.jpg",            
            "link": base_url('htmls/games/drag_drop/iceman.html'),
}








];

var itiration_data = js_data;








